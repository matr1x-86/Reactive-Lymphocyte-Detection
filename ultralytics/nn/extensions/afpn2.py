import sys
sys.path.append('/home/matr1x/workspace/paper_2/ultralytics')
...
class ASFF2(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.level == 0:                                             # 取大
            ...
            level_1_resized = self.stride_level_1(x_level_1)            # 小的上采样至大的
        elif self.level == 1:
            level_0_resized = self.stride_level_0(x_level_0)            # 大的下采样至小的
            level_1_resized = x_level_1

        level_0_weight_v = self.weight_level_0(level_0_resized)                     # 压缩得到8通道权重
        level_1_weight_v = self.weight_level_1(level_1_resized)                     # 压缩得到8通道权重
        levels_weight_v = torch.cat((level_0_weight_v, level_1_weight_v), 1)        # 拼接得到16通道权重
        levels_weight = self.weights_levels(levels_weight_v)                        # 压缩得到2通道权重
        levels_weight = F.softmax(levels_weight, dim=1)                             # 归一化
